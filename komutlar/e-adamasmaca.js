...
module.exports.run = async ( bot, message, args) => {
        ...
        if (oyndurum.has(message.channel.id)) return message.reply('<a:hayir:699131258378256465> Kanal başına sadece bir adam asmaca oyunu meydana gelebilir.');
        ...
        try {
            ...
            while (cevap.length !== confirmation.length && point < 6) {
                await message.channel.send(stripIndents`
                    ${displayText === null ? '**Devrim Adam Asmaca**!' : displayText ? '**Çok iyisin!**' : '**Yanlış Harf!**'}
                         **Kelime:**    \`${display.join(' ')}\`
                ...
                if (!guess.size) {
                    await message.channel.send('<a:hayir:699131258378256465> **Zamanın doldu!**');
                    break;
                }
            ...
            if (cevap.length === confirmation.length || tahmin) return message.channel.send(`**Tebrikler kelimeyi buldun!**: **${cevap} ** <a:like:695168724075806740>`);
            return message.channel.send(`<a:hayir:699131258378256465> **Maalesef bilemedin kelime bu:** **${cevap}**`);
        } catch (err) {
            oyndurum.delete(message.channel.id);
            return message.reply(`<a:hayir:699131258378256465> Olamaz! Bir Hata Verdi: \`${err.message}\``);
        }
